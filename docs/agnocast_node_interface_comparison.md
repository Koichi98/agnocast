...
| `get_node_names()` | ✓ | **Partial Support** | - | Returns the nodes of the caller's IPC namespace and `ROS_DOMAIN_ID`, from two sources: the agnocast nodes that own at least one non-bridge publisher/subscriber (from the kmod; beyond `MAX_NODE_NUM` (1024) of them the kmod side is dropped entirely and logged as an error), and the ROS 2 nodes that the `ros2agnocast_discovery_agent` reports (see [ROS 2 node visibility](#ros-2-node-visibility)). The caller itself is always reported. Duplicate node names are preserved, as in rclcpp |
...
- **A freshly started `rclcpp::Node` is briefly missing** whenever an agent is running. The kmod side is skipped for it the moment an agent's file exists, while the DDS side only carries it after the agent's next write. That file is rewritten once per second, so the gap is up to 1 s wide even if DDS discovery cost nothing, and discovery latency adds to it. Without an agent the same node appears in the kmod's list immediately; an `rclcpp::Node` the agent's participant never discovers stays missing for good.
