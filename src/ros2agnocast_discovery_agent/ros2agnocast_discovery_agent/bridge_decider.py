...
The request is sent as ``MqMsgDaemonBridge`` to the in-namespace bridge_manager
...
# Standard mode runs one bridge_manager per user process, keyed by that
# process's pid; performance mode runs one per namespace.
_STANDARD_MQ_PREFIX = '/agnocast_daemon_bridge@'
...
class BridgeRequest:
    ...
    # pid of the user process owning the local endpoint; addresses its
    # bridge_manager's MQ in standard mode. 0 when unknown (registry miss).
    target_pid: int
...
def decide_bridges(local_state, remote_states, registry=None) -> list:
    ...
    ``registry`` (optional TypeRegistryReader) resolves the owning process pid
    so standard-mode requests reach the right bridge_manager; without it the
    request carries ``target_pid=0`` and only the performance MQ receives it.
    Requests are collapsed to one per ``(topic, direction)``.
    """
    requests = {}

    def _owner_pid(topic_name: str, role: str, node_name: str) -> int:
        if registry is None:
            return 0
        entry = registry.lookup(topic_name, role, node_name)
        return getattr(entry, 'pid', 0) if entry is not None else 0
        ...
        for remote_topic in remote.topics:
            ...
            if local_pubs and remote_subs:
                ...
                requests.setdefault(key, BridgeRequest(
                    ...
                    target_pid=_owner_pid(local_topic.topic_name, 'pub', pub.node_name),
                ))
            ...
            if local_subs and remote_pubs:
                ...
                requests.setdefault(key, BridgeRequest(
                    ...
                    target_pid=_owner_pid(local_topic.topic_name, 'sub', sub.node_name),
                ))
...
def dispatch_requests(requests: Iterable[BridgeRequest], logger=None) -> None:
    """Deliver each request to the performance MQ and the owner's standard MQ.

    The owner's standard MQ is targeted only when its pid is known. Both modes
    are attempted because the daemon does not know which is active; the
    inactive mode's MQ is simply absent (ENOENT) and skipped.
    """
    ...
    for req in requests:
        ...
        targets = [perf_mq]
        if req.target_pid > 0:
            targets.append(_STANDARD_MQ_PREFIX + str(req.target_pid))
