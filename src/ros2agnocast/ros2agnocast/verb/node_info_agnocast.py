...
class NodeInfoAgnocastVerb(VerbExtension):
    ...
    def main(self, *, args):
        ...
        with NodeStrategy(None) as node:
            ...
            def get_agnocast_node_endpoints(topic_list, node_name):
                """Return (sub_topics, pub_topics, server_names, client_names) owned by `node_name`.

                server/client names are extracted from special service request/response topic
                names and are NOT topic names.
                """
                sub_topic_set = set()
                pub_topic_set = set()
                server_set = set()
                ...
                for topic_name in topic_list:
                    ...
                    # Service request/response topics were already accounted for above;
                    # no need to query their publishers.
                    if (
            ...
            def get_ros2_node_agnocast_endpoints(node_name_bytes):
                """Return (sub_topics, pub_topics, server_names, client_names) owned by `node_name_bytes`.
            ...
            with agnocast_topic_array(lib.get_agnocast_topics) as agnocast_topics:
                agnocast_subscribers, agnocast_publishers, agnocast_servers, agnocast_clients = get_agnocast_node_endpoints(agnocast_topics, node_name)
            ...
            if node_name in ros2_node_names:
                ...
                agnocast_subscribers, agnocast_publishers, agnocast_servers, agnocast_clients = get_ros2_node_agnocast_endpoints(node_name_bytes)
