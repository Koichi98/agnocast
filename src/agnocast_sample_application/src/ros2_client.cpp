...
int main(int argc, char * argv[])
{
  ...
  auto node = rclcpp::Node::make_shared("ros2_client");
  ...
  auto request1 = std::make_shared<agnocast_sample_interfaces::srv::SumIntArray::Request>();
  for (size_t i = 1; i <= ARRAY_SIZE; ++i) {
    request1->data.push_back(i);
  }
  client->async_send_request(
    request1, [node = node.get()](
  ...
  RCLCPP_INFO(node->get_logger(), "Sent request1");

  auto request2 = std::make_shared<agnocast_sample_interfaces::srv::SumIntArray::Request>();
  for (size_t i = 0; i < ARRAY_SIZE; ++i) {
    request2->data.push_back(i);
  }
  auto future = client->async_send_request(request2);
  RCLCPP_INFO(node->get_logger(), "Sent request2");
