...
class AgnocastPublisherOnlyPubsubTest : public rclcpp::Node
{
  ...
  std::vector<rclcpp::Subscription<MessageT>::SharedPtr> subscriptions_;

  void timer_callback()
  {
    for (size_t topic_index = 0; topic_index < publishers_.size(); ++topic_index) {
      agnocast::ipc_shared_ptr<MessageT> message =
        publishers_[topic_index]->borrow_loaned_message();
      message->id = count_;
  ...
  void subscription_callback(const MessageT::SharedPtr message, const size_t topic_index)
  {
...
public:
  AgnocastPublisherOnlyPubsubTest() : Node("agnocast_publisher_only_pubsub_test")
  {
    count_ = 0;
    ...
    for (int i = 0; i < topic_count; ++i) {
      ...
      auto publisher = agnocast::create_publisher<MessageT>(this, pub_topic_name, 1);

      publishers_.push_back(publisher);
      RCLCPP_INFO(
        this->get_logger(), "created agnocast publisher for topic: %s", pub_topic_name.c_str());
    }

    for (int i = 0; i < topic_count; ++i) {
      const std::string sub_topic_name = sub_topic_prefix + "_" + std::to_string(i);
      auto subscription = this->create_subscription<MessageT>(
        sub_topic_name, 1, [this, i](const MessageT::SharedPtr message) {
          this->subscription_callback(message, static_cast<size_t>(i));
        });

      subscriptions_.push_back(subscription);
      RCLCPP_INFO(
        this->get_logger(), "created rclcpp subscriber for topic: %s", sub_topic_name.c_str());
    }

    timer_ = this->create_wall_timer(
      100ms, std::bind(&AgnocastPublisherOnlyPubsubTest::timer_callback, this));
  }
};
...
int main(int argc, char * argv[])
{
  ...
  auto node = std::make_shared<AgnocastPublisherOnlyPubsubTest>();
