...
class MinimalPublisher : public rclcpp::Node
{
  ...
  void timer_callback()
  {
    ...
    // RCLCPP_INFO(this->get_logger(), "publish message: id=%ld to %zu topics", count_++,
    // publishers_.size());
  }
...
public:
  MinimalPublisher() : Node("cie_publisher")
  {
    ...
    for (int i = 0; i < topic_count; ++i) {
      ...
      RCLCPP_INFO(this->get_logger(), "created publisher for topic: %s", pub_topic_name.c_str());
    }
    ...
    for (int i = 0; i < topic_count; ++i) {
      ...
      RCLCPP_INFO(this->get_logger(), "created subscriber for topic: %s", sub_topic_name.c_str());
    }
