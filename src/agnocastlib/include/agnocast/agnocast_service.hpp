...
static_assert(to_event_publisher_role(ServiceRole::Default) == PublisherRole::Default);
static_assert(
  to_event_publisher_role(ServiceRole::BridgeInternal) == PublisherRole::BridgeInternal);
...
public:
   ...
   * if @p qos_service_event_pub uses a history policy Agnocast cannot support. The QoS is only
   * checked when it is about to be used, so disabling never rejects it.
