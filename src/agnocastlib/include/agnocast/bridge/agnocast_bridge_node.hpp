...
// Registers a bridge that forwards requests from Agnocast to ROS 2 (A2R).
struct AgnocastToRosServiceRegistrationPolicy
{
  template <typename ServiceT>
  static void register_bridge(const std::string & service_name)
  {
    register_service_bridge_core<ServiceT>(
...
{
  using AgnoClient = agnocast::BasicClient<ServiceT, NoBridgeRegistrationPolicy>;
