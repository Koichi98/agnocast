...
std::set<std::string> collect_external_ros2_client_service_names(rclcpp::Node * node)
{
  ...
  for (const auto & full_name : node->get_node_names()) {
    ...
    // A node can leave the graph between get_node_names() and this query. Losing that node's
    // clients beats losing the whole snapshot, which would read as "no demand" for every service.
    try {
