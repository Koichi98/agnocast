...
// ever assert on the two service names they own.
std::set<std::string> collect_until_peer_is_visible(rclcpp::Node * container)
{
  const auto deadline = std::chrono::steady_clock::now() + 10s;

  while (true) {
    auto names = agnocast::collect_external_ros2_client_service_names(container);
    if (names.count(PEER_SERVICE) > 0 || std::chrono::steady_clock::now() >= deadline) {
      return names;
    }
...
TEST_F(CollectExternalRos2ClientServiceNamesTest, reports_a_client_held_by_another_node)
{
  auto peer_client = peer_->create_client<rcl_interfaces::srv::GetParameters>(PEER_SERVICE);

  EXPECT_EQ(collect_until_peer_is_visible(container_.get()).count(PEER_SERVICE), 1U);
}
...
TEST_F(CollectExternalRos2ClientServiceNamesTest, skips_a_client_held_by_the_given_node)
{
  ...
  const auto names = collect_until_peer_is_visible(container_.get());
